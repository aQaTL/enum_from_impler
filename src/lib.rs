...
use quote::quote;
use syn::{Data, DeriveInput, Fields, Meta, Variant, Type, MetaList, NestedMeta, TypePath};
...
pub fn create_enum_from_impls(input: TokenStream) -> TokenStream {
    ...
    for variant in data.variants {
        ...
        let foreign_type = variant.attrs.iter().find_map(|attr| match attr.parse_meta() {
            Ok(Meta::Path(ref path)) => {
                if !path.get_ident().map(|v| v == "impl_from").unwrap_or_default() {
                    return None;
                }
			...
			Ok(Meta::List(MetaList{path, nested, ..})) => {
                ...
                match nested.into_iter().next() {
                    Some(NestedMeta::Meta(Meta::Path(path))) => Some(Type::Path(TypePath{
                        qself: None,
                        path,
                    })),
                    _ => None,
